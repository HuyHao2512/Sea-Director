...
export const createNewSeriesProject = (title?: string): SeriesProject => {
  ...
  const autoTitle = `Dự án mới ${datePart} ${timePart}`;
...
export const createNewProjectState = (): ProjectState => {
  return createNewEpisode('', '', 1, 'Dự án chưa đặt tên');
};
...
export const importIndexedDBData = async (
  ...
  if (!isValidExportPayload(payload)) throw new Error('Định dạng tệp nhập không chính xác');
    ...
    if (payload.stores.projects && payload.stores.projects.length > 0 && !(payload.stores.episodes && payload.stores.episodes.length > 0)) {
      payload.stores.projects.forEach((p: any) => {
        ...
        const sp: SeriesProject = {
          ...
          visualStyle: p.visualStyle || '3d-animation', language: p.language || 'Tiếng Việt',
        ...
        const s: Series = { id: seriesId, projectId, title: 'Mùa 1', sortOrder: 0, createdAt: Date.now(), lastModified: Date.now() };
        seriesStr.put(s);
        ...
        const ep: Episode = {
          ...
          language: p.language || 'Tiếng Việt', visualStyle: p.visualStyle || '3d-animation',
...
export const convertImageToBase64 = (file: File): Promise<string> => {
  return new Promise((resolve, reject) => {
    if (!file.type.startsWith('image/')) { reject(new Error('Chỉ hỗ trợ tệp hình ảnh')); return; }
    if (file.size > 10 * 1024 * 1024) { reject(new Error('Kích thước hình ảnh không được vượt quá 10MB')); return; }
