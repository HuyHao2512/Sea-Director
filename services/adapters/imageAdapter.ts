...
export const callImageApi = async (
  ...
  // ==== KIE (Nano Banana Async) API LOGIC ====
  if (activeModel.id === 'nano-banana-2' || activeModel.providerId === 'kie') {
    const createRes = await fetch(`${apiBase}/api/v1/jobs/createTask`, {
      method: 'POST',
      headers: {
    ...
    if (!createRes.ok) {
      throw buildImageApiError(createRes.status, await parseHttpErrorBody(createRes));
    }
    const createData = await createRes.json();
    const taskId = createData.data?.taskId;
    if (!taskId) throw new Error('Failed to create KIE image task');
    ...
    while (Date.now() - startTime < 300000) { // 5 minutes max
      ...
      if (state === 'success') {
        ...
        throw new Error('Task success but no URLs returned');
      } else if (state === 'fail') {
        throw new Error('KIE image generation failed: ' + statusData.data?.failMsg);
      }
    }
    
    if (!imageUrl) throw new Error('KIE task timed out');
    
    // Fetch result and transform to base64 to ensure it behaves consistently
    const imgRes = await fetchMediaWithCorsFallback(imageUrl);
    if (!imgRes.ok) throw new Error('Failed to download generated image via CORS proxy');
    ...
    return new Promise((resolve, reject) => {
       ...
       reader.onloadend = () => {
         ...
         if (result.startsWith('data:')) {
           ...
           reject(new Error('Base64 conversion parsing failed'));
         }
       };
       reader.onerror = () => reject(new Error('Image conversion via FileReader failed'));
       reader.readAsDataURL(blob);
    });
  }

  // ==== DEFAULT GEMINI SYNC API LOGIC ====
  // 构建请求 parts
