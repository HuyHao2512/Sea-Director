...
/**
 * 调用 KIE Video API（异步模式）
 */
const callKieVideoApi = async (
  options: VideoGenerateOptions,
  model: VideoModelDefinition,
  apiKey: string,
  apiBase: string
  ...
  const aspectRatio = options.aspectRatio || model.params.defaultAspectRatio;
  const imageUrls = [options.startImage, options.endImage].filter(Boolean) as string[];

  console.log(`🎬 Sử dụng KIE Video (${model.id}, ${aspectRatio})...`);

  // Tạo task
  const createRes = await fetch(`${apiBase}/api/v1/veo/generate`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${apiKey}`,
  ...
  if (!createRes.ok) {
    let errorMessage = `Tạo task video KIE thất bại: HTTP ${createRes.status}`;
    try {
      const errorData = await createRes.json();
      errorMessage = errorData.msg || errorData.error?.message || errorMessage;
    } catch (e) {
  ...
  if (!taskId) throw new Error('Không nhận được Task ID từ KIE.');

  console.log('📋 Task video KIE đã được tạo, ID:', taskId);

  // Polling kết quả qua recordInfo
  const startTime = Date.now();
  const maxPollingTime = 1200000; // 20 phút
  let videoUrl = '';

  while (Date.now() - startTime < maxPollingTime) {
    await new Promise(r => setTimeout(r, 5000)); // Polling mỗi 5 giây
    
    const statusRes = await fetch(`${apiBase}/api/v1/jobs/recordInfo?taskId=${taskId}`, {
      method: 'GET',
    ...
    console.log('🔄 Trạng thái Task video KIE:', state);

    if (state === 'success') {
      const resultStr = statusData.data?.resultJson;
      if (resultStr) {
      ...
      throw new Error('Task thành công nhưng không tìm thấy URL video.');
    } else if (state === 'fail') {
      throw new Error('Tạo video KIE thất bại: ' + statusData.data?.failMsg);
    }
  }

  if (!videoUrl) throw new Error('Task video KIE bị quá thời gian (Timeout).');
...
export const callVideoApi = async (
  ...
  // Theo Provider/Model để chọn API tương ứng
  if (activeModel.providerId === 'kie') {
    return callKieVideoApi(options, activeModel, apiKey, apiBase);
  } else if (activeModel.params.mode === 'async') {
