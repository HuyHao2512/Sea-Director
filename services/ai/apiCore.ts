...
/** Default API base URL fallback - Gemini */
...
export const getActiveChatModelName = (): string => {
  try {
    ...
    return model?.apiModel || model?.id || 'gemini-2.0-flash';
  } catch {
...
/** Non-stream chat completion - Gemini native format */
export const chatCompletion = async (
  ...
  const requestModel = resolveRequestModel('chat', model) || 'gemini-2.0-flash';
  ...
  try {
    // Gemini REST: /v1beta/models/{model}:generateContent?key={apiKey}
    const url = `${apiBase}/v1beta/models/${requestModel}:generateContent?key=${apiKey}`;
...
/** Streaming chat completion (SSE) - Gemini native format */
export const chatCompletionStream = async (
  ...
  try {
    // Gemini streaming: streamGenerateContent?alt=sse&key={key}
    const url = `${apiBase}/v1beta/models/${requestModel}:streamGenerateContent?alt=sse&key=${apiKey}`;
...
/** Verify whether a provided Gemini API key is valid */
export const verifyApiKey = async (key: string): Promise<{ success: boolean; message: string }> => {
  try {
    const apiBase = getApiBase('chat');
    // Gemini native verification
    const url = `${apiBase}/v1beta/models/gemini-2.0-flash:generateContent?key=${key}`;
    const response = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (data.candidates?.[0]?.content?.parts?.[0]?.text !== undefined) {
      return { success: true, message: 'Gemini API Key xác thực thành công!' };
    }

    return { success: false, message: 'Phản hồi từ API không đúng định dạng.' };
  } catch (error: any) {
...
export const resizeImageToSize = async (base64Data: string, targetWidth: number, targetHeight: number): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      console.error('Failed to load image for resizing. Input type:', base64Data.startsWith('http') ? 'URL' : 'Base64/Other');
      reject(new Error('Failed to load image'));
    };

    if (base64Data.startsWith('http') || base64Data.startsWith('blob:') || base64Data.startsWith('data:')) {
      img.src = base64Data;
    } else {
