...
export const parseScriptStructure = async (
  ...
  logScriptProgress('Đang phân tích cấu trúc kịch bản...');
...
export const enrichScriptDataVisuals = async (
  ...
  logScriptProgress(`Đang tạo prompt hình ảnh nhân vật/bối cảnh/đạo cụ (phong cách: ${nextData.visualStyle})...`);
  ...
  for (const idx of missingCharacterIndexes) {
    ...
    try {
      ...
      logScriptProgress(`Tạo prompt hình ảnh nhân vật: ${characters[idx].name}`);
  ...
  for (const idx of sceneIndexes) {
    ...
    try {
      ...
      logScriptProgress(`Tạo prompt hình ảnh bối cảnh: ${scenes[idx].location}`);
      const prompts = await generateVisualPrompts(
        'scene',
        scenes[idx],
        genre,
  ...
  for (const idx of propIndexes) {
    ...
    try {
      ...
      logScriptProgress(`Tạo prompt hình ảnh đạo cụ: ${props[idx].name}`);
      const prompts = await generateVisualPrompts(
        'prop',
        props[idx],
        genre,
  ...
  logScriptProgress('Tạo prompt hình ảnh hoàn tất');
  return nextData;
};
...
const applyScriptStageQualityPipeline = (
  ...
  logScriptProgress(`Kiểm tra chất lượng phân cảnh hoàn tất: ${repairedShots.length} cảnh (warning ${warnings}, fail ${fails})`);
...
export const generateShotList = async (
  ...
  logScriptProgress('Đang tạo danh sách phân cảnh...');
...
` : '';
  ...
  if (shouldReuseUnchangedScenes && previousScriptData && resolvePreviousSceneActionText) {
    ...
    if (reusableSceneBuckets.size > 0) {
      logScriptProgress(`Phát hiện ${reusableSceneBuckets.size} nhóm bối cảnh có thể tái sử dụng, sẽ ưu tiên dùng lại trong quá trình tạo.`);
    }
  }
  ...
  const processScene = async (scene: Scene, index: number): Promise<Shot[]> => {
    ...
    if (shouldReuseUnchangedScenes && reusableSceneBuckets.size > 0) {
      ...
      if (candidateGroup && candidateGroup.length > 0) {
        ...
        logScriptProgress(`Bối cảnh「${scene.location}」khớp tái sử dụng, bỏ qua tạo AI (tái sử dụng ${remapped.length} cảnh)`);
        return remapped;
      }
    }
    ...
    if (actionSource.source !== 'direct') {
      ...
      logScriptProgress(`Bối cảnh「${scene.location}」thiếu ánh xạ đoạn, đã dùng chiến lược bổ sung từ ${actionSource.source}`);
    }
  ...
  if (!enableQualityCheck) {
    logScriptProgress('Đã tắt kiểm tra chất lượng phân cảnh, bỏ qua chấm điểm và sửa tự động.');
  }
  logScriptProgress(`Tạo phân cảnh hoàn tất, tổng thời gian ${Math.round((Date.now() - overallStartTime) / 1000)}s`);
