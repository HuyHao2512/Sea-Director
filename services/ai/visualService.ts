 ...
 * Includes art direction document generation, character/scene visual prompt generation, image generation
 */
...
/**
 * Generate global art direction brief
 * Call before generating any character/scene visual prompts to establish unified visual style baseline for entire project.
 */
export const generateArtDirection = async (
  ...
  console.log('🎨 generateArtDirection called - generating global art direction document');
  logScriptProgress('Generating art direction document...');
...
Output ONLY valid JSON with this exact structure:
{
  ...
  try {
    ...
    console.log('✅ Global art direction document generation completed:', artDirection.moodKeywords.join(', '));
    logScriptProgress('Art direction document created successfully');
    return artDirection;
  } catch (error: any) {
    console.error('❌ Global art direction document generation failed:', error);
    logScriptProgress('Art direction document creation failed, using default style');
    return {
...
// ============================================
// Character Visual Prompt Batch Generation
// ============================================

/**
 * Batch generate visual prompts for all characters (Batch-Aware Generation)
 */
export const generateAllCharacterPrompts = async (
  ...
  logScriptProgress(`Đang tạo hàng loạt prompt hình ảnh cho ${characters.length} nhân vật (chế độ đồng nhất phong cách)...`);
...
Output ONLY the JSON, no explanations.`;
  ...
  try {
    ...
    logScriptProgress(`Tạo hàng loạt prompt nhân vật hoàn tất (${results.filter(r => r.visualPrompt).length}/${characters.length})`);
    return results;
  } catch (error: any) {
    console.error('❌ 批量角色视觉提示词生成失败:', error);
    logScriptProgress('Tạo hàng loạt prompt nhân vật thất bại, chuyển sang chế độ tạo từng cái');
...
const resolveImageModelRoutingFamily = (model: any): ImageModelRoutingFamily => {
  const identity = `${model?.id || ''} ${model?.apiModel || ''} ${model?.name || ''} ${model?.providerId || ''}`.toLowerCase();
  const isGeminiFamily =
    identity.includes('gemini') ||
    identity.includes('imagen') ||
    identity.includes('nano banana');
  return isGeminiFamily ? 'nano-banana' : 'generic';
};
...
NEGATIVE PROMPT (strictly avoid): ${compactNegativePrompt}`;
    ...
    // ==== DEFAULT GEMINI (Imagen 3) SYNC API LOGIC ====
    ...
    const response = await retryOperation(async () => {
      ...
      const url = `${apiBase}${imageModelEndpoint}?key=${apiKey}`;
    ...
    // Xử lý response format custom: {"code": 0, "message": "success", "data": {"taskId": "...", "outputImageUrls": [...]}}
    if (response.code === 0 && response.data?.outputImageUrls && Array.isArray(response.data.outputImageUrls)) {
      const imageUrl = response.data.outputImageUrls[0];
      
      addRenderLogWithTokens({
        type: 'keyframe',
        resourceId: 'image-' + Date.now(),
        resourceName: (typeof prompt === 'string' ? prompt : '').substring(0, 50) + '...',
        status: 'success',
        model: imageModelId,
    ...
    // Xử lý response format Gemini: base64 inline data
...
export const generateCharacterTurnaroundPanels = async (
  ...
  logScriptProgress(`Đang tạo mô tả góc nhìn 9 ô cho nhân vật「${character.name}」...`);
...
4) output JSON only, no explanation`;
    ...
    logScriptProgress(`Mô tả 9 ô nhân vật「${character.name}」đã hoàn tất`);
    return panels;
  } catch (error: any) {
    console.error(`❌ 角色 ${character.name} 九宫格视角描述生成失败:`, error);
    logScriptProgress(`Tạo mô tả 9 ô nhân vật「${character.name}」thất bại`);
    throw error;
  }
};
...
export const generateCharacterTurnaroundImage = async (
  ...
  logScriptProgress(`Đang tạo ảnh 9 ô trang phục cho nhân vật「${character.name}」...`);

  // 1. Luôn dùng Text-to-Image (T2I) theo yêu cầu của người dùng để tránh bị phụ thuộc vào ảnh cũ (Request #7)
...
Top priority: the character must look like the same person in all 9 panels.`;
  ...
  // Lưu ý: Nếu user muốn dùng Text-to-Image thuần túy cho nhân vật (User Request #7), 
  // chúng ta có thể bỏ qua referenceImage ở đây. 
  // Hiện tại vẫn giữ để đảm bảo tính nhất quán của nhân vật đã tạo trước đó.
  if (referenceImage) {
  ...
  try {
    ...
    logScriptProgress(`Ảnh 9 ô nhân vật「${character.name}」đã hoàn tất`);
    return imageUrl;
  } catch (error: any) {
    console.error(`❌ 角色 ${character.name} 九宫格造型图片生成失败:`, error);
    logScriptProgress(`Tạo ảnh 9 ô nhân vật「${character.name}」thất bại`);
    throw error;
  }
};
