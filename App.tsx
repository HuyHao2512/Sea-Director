  ...
  return (
    <div className="h-screen bg-[var(--bg-base)] flex items-center justify-center p-6">
      <div className="max-w-md text-center space-y-6">
        ...
        <h1 className="text-2xl font-bold text-[var(--text-primary)] mb-2">AI Director</h1>
        <div className="bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl p-8">
          <p className="text-[var(--text-tertiary)] text-base leading-relaxed mb-4">Để có trải nghiệm tốt nhất, vui lòng sử dụng trình duyệt trên máy tính (PC).</p>
          <p className="text-[var(--text-muted)] text-sm">Ứng dụng này yêu cầu không gian màn hình lớn và môi trường trình duyệt máy tính để hoạt động bình thường.</p>
        </div>
      </div>
    </div>
  );
...
function EpisodeWorkspace() {
  ...
  useEffect(() => {
    ...
    const isProjectPlaceholder =
      !projectTitle ||
      projectTitle === 'Dự án chưa đặt tên' ||
      /^Dự án mới\s\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}$/.test(projectTitle);
    ...
    if (/^Tập\s*\d+$/u.test(candidateTitle)) return;
  ...
  const setStage = (stage: 'script' | 'assets' | 'director' | 'export' | 'prompts') => {
    if (isGenerating) {
      showAlert('Hiện đang có tác vụ tạo nội dung đang chạy, việc chuyển trang có thể làm mất dữ liệu.\n\nBạn có chắc chắn muốn rời khỏi trang này không?', {
        title: 'Tác vụ đang chạy', type: 'warning', showCancel: true, confirmText: 'Rời khỏi', cancelText: 'Tiếp tục đợi',
        onConfirm: () => {
  ...
  const handleExit = async () => {
    if (isGenerating) {
      showAlert('Hiện đang có tác vụ tạo nội dung đang chạy, việc thoát ngay có thể làm mất dữ liệu.\n\nBạn có chắc chắn muốn thoát không?', {
        title: 'Tác vụ đang chạy', type: 'warning', showCancel: true, confirmText: 'Xác nhận thoát', cancelText: 'Tiếp tục đợi',
        onConfirm: async () => {
          setIsGenerating(false);
          if (currentEpisode) {
  ...
  if (!currentEpisode) {
    return <div className="h-screen flex items-center justify-center text-[var(--text-muted)]">Đang tải...</div>;
  }
  ...
  const renderStage = () => {
    switch (currentEpisode.stage) {
      ...
      default:
        return <div className="text-[var(--text-primary)]">Giai đoạn chưa xác định</div>;
    }
  };
      ...
      <main className="ml-72 flex-1 h-screen overflow-hidden relative">
        ...
        {showSaveStatus && (
          <div className="absolute top-4 right-6 pointer-events-none flex items-center gap-2 text-xs font-mono text-[var(--text-tertiary)] bg-[var(--overlay-medium)] px-2 py-1 rounded-full backdrop-blur-sm z-50">
            {saveStatus === 'saving' ? (<><Save className="w-3 h-3 animate-pulse" />Đang lưu...</>) : (<><CheckCircle className="w-3 h-3 text-[var(--success)]" />Đã lưu</>)}
          </div>
        )}
      </main>
...
function AppRoutes() {
  ...
  useEffect(() => {
    const handleError = (event: ErrorEvent) => {
      if (event.error?.message?.includes('API Key missing') || event.error?.message?.includes('Gemini API Key')) {
        setShowModelConfig(true); event.preventDefault();
      }
    };
    const handleRejection = (event: PromiseRejectionEvent) => {
      if (event.reason?.message?.includes('API Key missing') || event.reason?.message?.includes('Gemini API Key')) {
        setShowModelConfig(true); event.preventDefault();
      }
    };
  ...
  const handleSaveApiKey = (key: string) => {
    if (key) { setApiKeyState(key); setGlobalApiKey(key); localStorage.setItem('gemini_api_key', key); }
    else { setApiKeyState(''); setGlobalApiKey(''); localStorage.removeItem('gemini_api_key'); }
  };
