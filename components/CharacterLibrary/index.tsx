...
const TABS: Array<{ key: LibraryAssetType; label: string }> = [
  { key: 'character', label: 'Thư viện nhân vật' },
  { key: 'scene', label: 'Thư viện cảnh' },
  { key: 'prop', label: 'Thư viện đạo cụ' },
];
...
const CharacterLibraryPage: React.FC = () => {
  ...
  const confirmDeleteAsset = (
    ...
    const msg =
      refCount > 0
        ? `${assetTypeLabel}“${assetName}”已被 ${refCount} tập tham chiếu，删除后各集中的引用副本将变为独立${assetTypeLabel}。确定删除？`
  ...
  const uploadAssetImage = async <T extends Character | Scene | Prop>(
    ...
    try {
      ...
      showAlert(`Tải lên thất bại: ${error instanceof Error ? error.message : 'Lỗi không xác định'}`, { type: 'error' });
    }
  };

  const handleDeleteCharacter = (character: Character) =>
    confirmDeleteAsset('Nhân vật', character.name, getCharacterRefCount(character.id), () => removeCharacterFromLibrary(character.id));

  const handleDeleteScene = (scene: Scene) =>
    confirmDeleteAsset('Cảnh', scene.location, getSceneRefCount(scene.id), () => removeSceneFromLibrary(scene.id));

  const handleDeleteProp = (prop: Prop) =>
    confirmDeleteAsset('Đạo cụ', prop.name, getPropRefCount(prop.id), () => removePropFromLibrary(prop.id));
  ...
  const handleAddAsset = () => {
    if (activeTab === 'character') {
      ...
      const nextCharacter: Character = {
        ...
        gender: newCharacterForm.gender.trim() || 'Chưa xác định',
        age: newCharacterForm.age.trim() || 'Chưa xác định',
    ...
    if (activeTab === 'scene') {
      ...
      const nextScene: Scene = {
        ...
        time: newSceneForm.time.trim() || 'Thời gian chưa xác định',
        atmosphere: newSceneForm.atmosphere.trim() || 'Thông thường',
        visualPrompt: '',
        version: 1,
      };
    ...
    const nextProp: Prop = {
      ...
      category: newPropForm.category.trim() || 'Khác',
    ...
    if (filteredCharacters.length === 0) {
      return (
        <div className="border border-dashed border-[var(--border-primary)] p-12 text-center text-[var(--text-muted)]">
          ...
          <p className="text-sm mb-2">{searchQuery ? 'Không tìm thấy nhân vật' : 'Thư viện nhân vật trống'}</p>
    ...
    if (filteredScenes.length === 0) {
      return (
        <div className="border border-dashed border-[var(--border-primary)] p-12 text-center text-[var(--text-muted)]">
          ...
          <p className="text-sm mb-2">{searchQuery ? 'Không tìm thấy cảnh' : 'Thư viện cảnh trống'}</p>
    ...
    if (filteredProps.length === 0) {
      return (
        <div className="border border-dashed border-[var(--border-primary)] p-12 text-center text-[var(--text-muted)]">
          ...
          <p className="text-sm mb-2">{searchQuery ? 'Không tìm thấy đạo cụ' : 'Thư viện đạo cụ trống'}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--bg-base)]/70 p-6" onClick={() => setShowAddModal(false)}>
          <div className="w-full max-w-md bg-[var(--bg-primary)] border border-[var(--border-primary)] p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-bold text-[var(--text-primary)] uppercase tracking-widest mb-4">
              {activeTab === 'character' ? 'Thêm nhân vật vào thư viện' : activeTab === 'scene' ? 'Thêm cảnh vào thư viện' : 'Thêm đạo cụ vào thư viện'}
            </h3>
