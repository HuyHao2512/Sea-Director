...
const ProjectAssetPicker: React.FC<Props> = ({ isOpen, onClose, project, assetType, existingIds, onSelectScene, onSelectProp }) => {
  ...
  const title = isScene ? 'Thêm từ thư viện cảnh' : 'Thêm từ thư viện đạo cụ';
  const emptyText = isScene ? 'Thư viện cảnh trống' : 'Thư viện đạo cụ trống';
      ...
      <div className="w-full max-w-2xl bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-xl overflow-hidden max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-3 border-b border-[var(--border-subtle)]">
          <div className="relative">
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder={isScene ? 'Tìm kiếm cảnh...' : 'Tìm kiếm đạo cụ...'} className="w-full pl-9 pr-3 py-2 bg-[var(--bg-primary)] border border-[var(--border-primary)] text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)] outline-none rounded" autoFocus />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {items.length === 0 ? (
            <div className="text-center py-12 text-[var(--text-muted)]">
              ...
              <p className="text-xs">{query ? 'Không tìm thấy kết quả' : emptyText}</p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3">
                      ...
                      {linked && <div className="text-[9px] text-[var(--success-text)] mt-1 font-mono">Đã thêm</div>}
                    </div>
                  </button>
                );
              })}
