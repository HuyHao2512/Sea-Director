...
const StageDirector: React.FC<Props> = ({ project, updateProject, onApiKeyError, onGeneratingChange }) => {
  ...
  const [useAIEnhancement, setUseAIEnhancement] = useState(false); // Có sử dụng tăng cường AI prompt không
  const [isSplittingShot, setIsSplittingShot] = useState(false); // Có đang tách cảnh quay không
  const [showNineGrid, setShowNineGrid] = useState(false); // Có hiển thị cửa sổ xem trước lưới 9 ô không
  const [toastMessage, setToastMessage] = useState('');
  
  // Tỷ lệ ngang/dọc được sử dụng để tạo keyframe (đọc từ cấu hình lưu trữ)
  const [keyframeAspectRatio, setKeyframeAspectRatioState] = useState<AspectRatio>(() => getUserAspectRatio());
  
  // Bao bọc setKeyframeAspectRatio, đồng thời lưu trữ vào cấu hình mô hình
  const setKeyframeAspectRatio = (ratio: AspectRatio) => {
    setKeyframeAspectRatioState(ratio);
    setUserAspectRatio(ratio);
  };
  
  // Trạng thái chỉnh sửa thống nhất
  ...
  const formatUserFriendlyError = (error: any, fallback: string): string => {
    ...
    if (!normalizedMessage) {
      if (status === 400) {
        normalizedMessage = 'Prompt có thể bị chặn bởi kiểm soát nội dung, vui lòng sửa đổi và thử lại.';
      } else if (status === 500 || status === 503) {
        normalizedMessage = 'Máy chủ đang bận, vui lòng thử lại sau.';
      } else {
  ...
  const handleAIReassessQuality = async () => {
    ...
    try {
      ...
      const sourceLabel = assessment.version >= 2 ? 'AI V2' : 'Sửa lỗi luật V1';
      showAlert(`Điểm chất lượng đã được cập nhật (${sourceLabel}): ${assessment.score} điểm`, { type: 'success' });
    } catch (error: any) {
      if (onApiKeyError && onApiKeyError(error)) return;
      showAlert(`Đánh giá lại bằng AI thất bại: ${formatUserFriendlyError(error, 'Vui lòng thử lại sau.')}`, { type: 'error' });
    } finally {
  ...
  const handleGenerateKeyframe = async (shot: Shot, type: 'start' | 'end') => {
    ...
    const continuityHint = type === 'end' && startKf?.visualPrompt
      ? `[Ràng buộc liên quán] Khung hình kết thúc phải giữ nguyên danh tính nhân vật, trang phục, bối cảnh và logic ánh sáng của khung hình bắt đầu, đồng thời thể hiện sự thay đổi rõ rệt trong bố cục và kết quả hành động. Tham khảo khung hình bắt đầu: ${extractBasePrompt(startKf.visualPrompt, shot.actionSummary).slice(0, 200)}`
      : '';

    const basePrompt = continuityHint && !rawBasePrompt.includes('[Ràng buộc liên quán]')
      ? `${rawBasePrompt}\n\n${continuityHint}`
      : rawBasePrompt;
    ...
    if (useAIEnhancement) {
      try {
        ...
        console.error('AI tăng cường thất bại, sử dụng Prompt cơ bản:', error);
    ...
    try {
      ...
      showAlert(`Tạo thất bại: ${formatUserFriendlyError(e, 'Tạo hình ảnh thất bại, vui lòng thử lại sau.')}`, { type: 'error' });
    }
  };
  ...
  const handleUploadKeyframeImage = async (shot: Shot, type: 'start' | 'end') => {
    ...
    input.onchange = async (e: any) => {
      ...
      if (!file.type.startsWith('image/')) {
        showAlert('Vui lòng chọn tệp hình ảnh!', { type: 'warning' });
        return;
      }
      ...
      try {
        ...
        showAlert('Đọc tệp thất bại!', { type: 'error' });
      }
    };
  ...
  * @param duration - 视频时长（仅异步模型有效）
  ...
  const handleGenerateVideo = async (shot: Shot, aspectRatio: AspectRatio = '16:9', duration: VideoDuration = 8, modelId?: string) => {
    ...
    if (selectedModelRouting.family === 'veo-sync' && (!hasCompletedStartFrame || !hasCompletedEndFrame)) {
      return showAlert('Chế độ đầu-cuối của Veo 3.1 yêu cầu cả hình ảnh khung hình đầu và cuối phải hoàn tất, vui lòng bổ sung trước khi tạo video.', { type: 'warning' });
    }
    
    // 必须有起始帧
    if (!sKf?.imageUrl) {
      return showAlert("Vui lòng tạo khung hình bắt đầu trước!", { type: 'warning' });
    }
    ...
    if (routedFrames.ignoredEndFrame) {
      if (videoInputMode === 'storyboard-grid' && !!eKf?.imageUrl) {
        setToastMessage('Chế độ phân cảnh lưới đã bật: Video sẽ chỉ sử dụng khung hình đầu, khung hình cuối đã bị tự động bỏ qua.');
      } else {
        const modelName = selectedModelRouting.family === 'sora'
          ...
          : selectedModelRouting.family === 'doubao-task'
            ? 'Doubao Task'
            : selectedModel;
        setToastMessage(`Định tuyến năng lượng: ${modelName} hiện chỉ sử dụng khung hình đầu, đã tự động bỏ qua khung hình cuối.`);
      }
    }
    ...
    if (!preflightResult.canProceed) {
      showAlert(`Kiểm tra trước video không vượt qua:\n${formatLintIssues(preflightResult.issues)}`, { type: 'warning' });
      return;
    }

    const nonErrorIssues = preflightResult.issues.filter((issue) => issue.severity !== 'error');
    if (nonErrorIssues.length > 0) {
      setToastMessage(`Nhắc nhở kiểm tra trước video:\n${formatLintIssues(nonErrorIssues)}`);
    }
    ...
    try {
      ...
      if (onApiKeyError && onApiKeyError(e)) return;
      showAlert(`Tạo video thất bại: ${formatUserFriendlyError(e, 'Vui lòng thử lại sau.')}`, { type: 'error' });
    }
  };
  ...
  const handleCopyPreviousEndFrame = () => {
    ...
    if (!previousEndKf?.imageUrl) {
      showAlert("Phân cảnh trước đó vẫn chưa tạo khung hình kết thúc", { type: 'warning' });
      return;
    }
  ...
  const handleCopyNextStartFrame = () => {
    ...
    if (!nextStartKf?.imageUrl) {
      showAlert("Phân cảnh tiếp theo vẫn chưa tạo khung hình bắt đầu", { type: 'warning' });
      return;
    }
  ...
  if (!project.shots.length) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-[var(--text-tertiary)] bg-[var(--bg-secondary)]">
        ...
        <p>Chưa có dữ liệu phân cảnh, vui lòng quay lại Giai đoạn 1 để tạo kịch bản phân cảnh.</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-[var(--bg-secondary)] relative overflow-hidden">
      ...
      <div className="h-16 border-b border-[var(--border-primary)] bg-[var(--bg-elevated)] px-6 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-[var(--text-tertiary)] uppercase">Tỷ lệ</span>
            <AspectRatioSelector
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-[var(--bg-base)]/30 border border-[var(--border-primary)]">
            ...
            <span className="text-xs text-[var(--text-tertiary)]">Tối ưu Prompt bằng AI</span>
              <input
                type="checkbox"
          ...
          <span className={`text-xs font-mono px-2 py-1 rounded border ${
            ...
            Điểm chất lượng {projectQualityScore}
          </span>
          ...
          >
            ...
            {allStartFramesGenerated ? 'Tạo lại tất cả首帧' : 'Tạo hàng loạt首帧'}
          </button>
        </div>
      </div>
      ...
      <EditModal
        ...
        title={
          ...
          editModal?.type === 'dialogue' ? 'Chỉnh sửa lời thoại' :
          editModal?.type === 'keyframe' ? 'Chỉnh sửa Prompt khung hình chính' :
        ...
        placeholder={
          editModal?.type === 'action' ? 'Mô tả hành động và nội dung phân cảnh...' :
          editModal?.type === 'dialogue' ? 'Nhập lời thoại (để trống nếu không có lời thoại)...' :
          editModal?.type === 'keyframe' ? 'Nhập Prompt cho khung hình chính...' :
