      ...
      <div className={STYLES.modal.container}>
        ...
        <div className={STYLES.modal.header}>
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl font-bold text-[var(--text-primary)]">Nhật ký render</h3>
        ...
        <div className={STYLES.statsPanel.container}>
          <div className={STYLES.statsPanel.grid}>
            <div className={STYLES.statsPanel.card}>
              <div className={STYLES.statsPanel.label}>Tổng bản ghi</div>
              <div className="text-2xl font-mono font-bold text-[var(--text-primary)]">{stats.total}</div>
            </div>
            <div className={STYLES.statsPanel.card}>
              <div className={STYLES.statsPanel.label}>Thành công</div>
        ...
        <div className={STYLES.modal.content}>
          {logs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-[var(--text-muted)]">
              ...
              <p className="text-sm font-mono uppercase tracking-widest">Chưa có lịch sử tạo</p>
            </div>
          ) : (
            logs.map((log) => {
                ...
                <div key={log.id} className={STYLES.logItem.container}>
                  ...
                  {isExpanded && hasDetails && (
                    <div className={STYLES.logItem.details}>
                      {log.resourceId && (
                        <div>
                          <div className="text-[9px] text-[var(--text-muted)] uppercase tracking-widest font-bold mb-1">Mã tài nguyên</div>
                      ...
                      {(log.inputTokens || log.outputTokens || log.totalTokens) && (
                        <div>
                          <div className="text-[9px] text-[var(--text-muted)] uppercase tracking-widest font-bold mb-1">Lượng token</div>
                          <div className="flex gap-4 text-[10px]">
                            {log.inputTokens && (
                              <div className="bg-[var(--bg-base)]/30 px-2 py-1 rounded">
                                <span className="text-[var(--text-tertiary)]">Đầu vào:</span>
                                <span className="text-[var(--accent-text)] font-mono ml-1">{log.inputTokens}</span>
                              </div>
                            )}
                            {log.outputTokens && (
                              <div className="bg-[var(--bg-base)]/30 px-2 py-1 rounded">
                                <span className="text-[var(--text-tertiary)]">Đầu ra:</span>
