...
const ModelConfigModal: React.FC<ModelConfigModalProps> = ({ isOpen, onClose }) => {
  ...
  const tabs: { id: TabType; label: string; icon: React.ReactNode }[] = [
    { id: 'global', label: 'Cài đặt chung', icon: <Key className="w-4 h-4" /> },
    { id: 'chat', label: 'Model hội thoại', icon: <MessageSquare className="w-4 h-4" /> },
    { id: 'image', label: 'Model hình ảnh', icon: <Image className="w-4 h-4" /> },
    { id: 'video', label: 'Model video', icon: <Video className="w-4 h-4" /> },
  ];
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[var(--border-subtle)] flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-[var(--text-primary)]">Cấu hình Model</h2>
        ...
        <div className="px-6 py-4 border-t border-[var(--border-subtle)] bg-[var(--bg-sunken)] rounded-b-xl flex-shrink-0 flex items-center justify-between">
          <p className="text-[10px] text-[var(--text-muted)] font-mono">
            Cấu hình chỉ lưu trên trình duyệt cục bộ
          </p>
          <button
            onClick={onClose}
