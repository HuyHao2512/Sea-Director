...
const ShotRow: React.FC<Props> = ({
  ...
  // Trích xuất số thứ tự hiển thị từ shot.id
  // Ví dụ: shot-1 → "SHOT 001", shot-1-1 → "SHOT 001-1"
  const getShotDisplayNumber = () => {
    const idParts = shot.id.split('-').slice(1); // Bỏ tiền tố "shot"
    if (idParts.length === 1) {
      // Cảnh quay chính: shot-1 → "SHOT 001"
      return `SHOT ${String(idParts[0]).padStart(3, '0')}`;
    } else if (idParts.length === 2) {
      // Cảnh quay con: shot-1-1 → "SHOT 001-1"
      return `SHOT ${String(idParts[0]).padStart(3, '0')}-${idParts[1]}`;
    } else {
      // Phương án dự phòng: dùng shotNumber được truyền vào
      ...
      <div className="flex-1 space-y-4">
        {editingShotActionId === shot.id ? (
          <div className="space-y-3 p-4 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest">Mô tả hành động</label>
              <textarea
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest">Thoại (tùy chọn)</label>
              <textarea
                ...
                rows={2}
                placeholder="Nhập thoại (để trống nếu không có)..."
              />
            </div>
          ...
          <div className="relative group/action">
            <div className="flex items-start gap-2">
              ...
              <button
                ...
                title="Chỉnh sửa hành động và thoại"
              >
        ...
        <div className="pt-2">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Nhân vật</span>
            <button
          ...
          {editingShotCharactersId === shot.id ? (
            <div className="space-y-3 p-3 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg">
              <div className="space-y-2">
                <div className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider">Nhân vật hiện tại</div>
                <div className="flex flex-wrap gap-2">
                  {shot.characters.length === 0 ? (
                    <span className="text-xs text-[var(--text-muted)] italic">Chưa có nhân vật</span>
                  ) : (
              ...
              <div className="space-y-2">
                <div className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider">Thêm nhân vật</div>
                <div className="flex flex-wrap gap-2">
                  ...
                  {scriptData?.characters.filter(char => !shot.characters.includes(char.id)).length === 0 && (
                    <span className="text-xs text-[var(--text-muted)] italic">Đã thêm tất cả nhân vật</span>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
              {shot.characters.length === 0 ? (
                <span className="text-[10px] text-[var(--text-muted)] italic">Chưa có nhân vật</span>
              ) : (
                shot.characters.map(cid => {
                  const char = scriptData?.characters.find(c => c.id === cid);
                  return char ? (
        ...
        <div className="xl:hidden pt-4 border-t border-[var(--border-subtle)]">
          <div className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 flex items-center gap-2 justify-between">
            <span className="flex items-center gap-2">
              <Aperture className="w-3 h-3" /> Lời nhắc trên màn hình
            </span>
